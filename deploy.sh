...
if [ -z "$AZURE_DEFAULTS_LOCATION" ]
then
    # Input location
    echo -n "Enter location (e.g. uksouth): "
...
if [ -z "$AZURE_DEFAULTS_GROUP" ]
then
    # Input prefix
    echo -n "Enter resource group name: "
    stty_orig=`stty -g` # save original terminal setting.
    read rg         # read the prefix
    stty $stty_orig     # restore terminal setting.
    if [ -z "$rg" ]
    then
        rg="rg-fortigate"
    ...
    rg="$AZURE_DEFAULTS_GROUP"
fi
echo ""
echo "--> Using resource group '$rg' for all resources ..."
echo ""
